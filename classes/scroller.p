...
^if($mode eq "html"){
	^if($type eq "separator"){
		$result[<span class="separator">$name</span>]
	}{
		^if($type ne "current" && def $page_num){
			$result[<span><a href="^untaint[html]{^if(def $url){$url}{./}^if($page_num != $first_page){${url_separator}$form_name=$page_num}}">$name</a></span>]]
		}{
			$result[<span class="active">$name</span>]
		}
	}
}{
	$result[<page
